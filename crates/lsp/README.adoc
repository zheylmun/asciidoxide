...
symbols.rs     ASG → DocumentSymbol conversion for document outline
folding.rs     ASG → FoldingRange conversion for collapsible regions
definition.rs  xref → block ID go-to-definition lookup
hover.rs       ASG traversal for contextual hover information
util.rs        Shared LSP position/range conversion helpers
----
